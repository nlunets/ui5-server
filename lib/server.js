...
module.exports = {
	...
	serve(tree, {port, changePortIfInUse = false, h2 = false, key, cert, acceptRemoteConnections = false}) {
		return Promise.resolve().then(() => {
			...
			// 	secure: false
			// }));
			if (tree.resources && tree.resources.proxy) {
				for (const proxyPath in tree.resources.proxy) {
					if (typeof tree.resources.proxy[proxyPath] == 'string') {
						app.use(proxyPath, proxy(proxyPath, {
							target: tree.resources.proxy[proxyPath],
							changeOrigin: true
						}));
					} else {
						app.use(proxyPath, proxy(proxyPath, tree.resources.proxy[proxyPath]))
					}
				}
			}
